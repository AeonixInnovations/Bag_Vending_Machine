...
import { postMarket } from "./market.controller";
...
export const registerNewDevice = async (req: Request, res: Response) => {
  ...
  const { device_id, address, machine_contact_number, date, marketName } =
    req.body;
  try {
    ...
    let marketResponse: any = null;
    if (marketName !== "") {
      marketResponse = await postMarket(marketName, address);
      if (!marketResponse) {
        return res.status(400).json({
          message: "Something went wrong",
        });
      }
      // payload.marketID = marketResponse._id;
    }
    let payload: Partial<DeviceInterface> = {
...
export const getAllDeviceList = async (req: Request, res: Response) => {
  ...
  try {
    const page: number = req.query.page
      ...
      : 1;
    const perPage: number = req.query.perPageItem
      ? parseInt(req.query.perPageItem as string, 10)
      : 10;
    ...
    const devices = await DeviceModel.find({}).skip(skip).limit(perPage);
...
export const fetchDeviceData = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const response = await DeviceModel.find({ device_id: device_id }).select(
      "-__v"
    );
    console.log(device_id);
    return res.status(200).json({
