...
export const getMongoDBFormattedFirstOfNextMonth = (): string => {
	...
	const formattedDate = newDate.getFullYear() + "-" + (newDate.getMonth() + 1) + "-" + 1;
...
export const formatDate = (date: Date | string): string => {
	...
	const MM =
		String(_date.getMonth() + 1).length == 1 ? "0" + String(_date.getMonth() + 1) : String(_date.getMonth() + 1);
	const DD = String(_date.getDate()).length == 1 ? "0" + _date.getDate() : String(_date.getDate());
...
export const dateDifference = (
	...
	const differenceInTime: number = ((date2 as number) - date1) as number as number;
...
export const calculateAge = (birthDate: string, otherDate: string | Date): number => {
...
export const getAgeInDays = (dateOfBirth: string) => {
	...
	const diffDays = Math.round(
		Math.abs(((firstDate as unknown as number) - (secondDate as unknown as number)) / oneDay)
	);
	return diffDays;
};
...
export const getFormattedMongoDBDateServiceForDBComparison = (_date: string) => {
	const date: string | Date = new Date(_date);
	const formattedDate = date.getFullYear() + "-" + (date.getMonth() + 1) + "-" + date.getDate();
	return formateMongoDateService(formattedDate);
};
...
export const ExcelDateToYYYYMMDD = (serial: number | string): string | Date => {
	...
	if (serial.toString().length > 7) {
		...
		const _date = tDate.getFullYear() + "-" + (tDate.getMonth() + 1) + "-" + tDate.getDate();
